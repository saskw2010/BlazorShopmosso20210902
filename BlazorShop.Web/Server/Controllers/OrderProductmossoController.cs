...
namespace BlazorShop.Server.Controllers
{
    ...
    [ApiController]
    public class OrderProductmossoController : ControllerBase
    {
        private readonly IOrdersProductServicemosso _ordersProductServicemosso;
        

        public OrderProductmossoController(IOrdersProductServicemosso OrdersProductServicemosso)
        {
            _ordersProductServicemosso = OrdersProductServicemosso;
        }

        [HttpGet]
        public async Task<ActionResult<List<OrderProduct>>> GetOrderProductslist()
        {
            return Ok(await _ordersProductServicemosso.GetOrderProductslist());
        }

        
        [HttpGet("{OrdersProductId}")]
        public async Task<ActionResult<OrderProduct>> GetOrderProductbyorderid(int OrdersProductId)
        {
            return Ok(await _ordersProductServicemosso.GetOrderProductbyorderid(OrdersProductId));
        }
