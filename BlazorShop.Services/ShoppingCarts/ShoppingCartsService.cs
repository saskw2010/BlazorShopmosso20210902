namespace BlazorShop.Services.ShoppingCarts
{
    ...
    public class ShoppingCartsService : BaseService<ShoppingCart>, IShoppingCartsService
    {
        ...
        public async Task<Result> ConfirmUserCart(List<ShoppingCartRequestModel> cartItems, string userId)
        {
            var exist = AllByUserId(userId).ToList();
            foreach (var item in exist)
            {
                var check = cartItems.FirstOrDefault(cc=>cc.ProductId==item.ProductId);
                if(check==null)
                {
                    exist = exist.Where(e => e.ProductId != item.ProductId).ToList();
                    this.Data.Remove(item);
                }
                else
                {
                    var productQuantity = await this.GetProductQuantityById(item.ProductId);

                    if (productQuantity < check.Quantity)
                    {
                        return NotEnoughProductsMessage;
                    }
                    item.Quantity = check.Quantity;
                    item.Notes = check.Notes;
